...
// scripts/tf-field-coverage-baseline.txt, the same shape as its sibling. That
// file is maintained by hand: this script reads it and never writes it. It used
// to, behind --update-baseline, which regenerated the file from the gap list
// and deleted every reason recorded in it (fogpipe/cloud-workspace#888) —
// including the "named differently on purpose" section, which is the one this
// gate cannot re-derive.
//
// Usage:
...
if (Deno.args.includes("--update-baseline")) {
  console.error(
    "--update-baseline is gone: it regenerated\n" +
      "scripts/tf-field-coverage-baseline.txt from the gap list and deleted\n" +
      "every reason recorded in it (fogpipe/cloud-workspace#888).\n" +
...
if (newGaps.length) {
  ...
  console.log(
    "\nAdd the attribute to the resource's schema and model. If it is\n" +
      ...
      "a write-only credential — paste the line(s) above into\n" +
      "scripts/tf-field-coverage-baseline.txt and write down why, in the group\n" +
      "it belongs to.",
  );
} else {
